...
These peaks are called "characteristic radiation." They are so named because their energy levels are unique to each target material, acting as a fingerprint that can be used to identify it. The highlights on the graph indicates characteristic radiation peaks for tungsten.<br><br>
{% include end-side-by-side.html %}
