...
The figure below shows a detailed view of the sinogram data. 
...
## 2-1) A $\xi-\eta$ coordinate system 
To understand how the Radon transform builds a sinogram, the key is to think in a new, rotated coordinate system, which we'll call $\xi-\eta$ coordinate. 
This is shown below. 
...
margin_bottom = "-0.5em"
%}
Let's introduce our second coordinate, $\xi-\eta$. 
The crucial part is that this system rotates with the X-ray source and detector. 
The angle of this rotation, relative to the fixed $(x, y)$, is $\gamma$ ($\in [0, 2\pi]$).
In this rotating $\xi-\eta$ coordinate system, we can rewrite the value on object, $f(x,y)$, into $\mu(\xi, \eta)$.

$$f(x,y) \mapsto \mu(\xi, \eta)$$
...
To make these calculation simple, we defined new $\xi-\eta$ coordinate.
{% include end-side-by-side.html %}
{% include end-box.html %}


## 2-2) Mathematical equation of Radon transform
So, how do we practically calculate $p(\xi, \gamma)$, the line integral of $f(x,y)$?

The mathematical definition is:

<div class="math-box" style="padding-top: 0.2em;">
$$p_{\gamma}(\xi) = p(\xi, \gamma) = \int_{\vec{r} \in L(x,y)} f(\vec{r}) \, d\vec{r}, \quad \text{where } \vec{r}=\begin{bmatrix}x \\ y \end{bmatrix} \tag{2.1}$$
</div>
...
Trying to do this directly in the fixed $(x, y)$ coordinate system is a repetitive and boring. 
...
Why is this better? Because in the rotated system, the line $L$ is no longer a complicated, angled line but just vertical line. 
...
To make this switch, we first need a relationship between the $(x, y)$ system and the $(\xi, \eta)$ system. 
...
{% include start-box.html class="math-box"%}
 ...
 margin_bottom = "-0.5em"
 %}
The coordinates of a point $(\xi, \eta)$ in the $\xi-\eta$ coordinate system can be expressed in the $x-y$ coordinate system as follows:
...
&= \int_{\vec{r} \in L(x,y)} f(\vec{r}) \mathrm{d}\vec{r}\\
&= \int f(\xi \cos \gamma - \eta \sin \gamma, \xi \sin \gamma + \eta \cos \gamma) \mathrm{d}\eta \\
...
First, let's back to **[4. Probability of detection]**. 

We derived that the expected number of photons that transmit through the object with $N_0$ incident photons is $\lambda = \int N_0 S_0(E) e^{-\int \mu_A(r, E) dr} dE$. This equation can be reformulated in the $\xi-\eta$ coordinate system as follows:

<div class="math-box" style="padding-top: 0.2em;">
$$\text{Number of transmitted photons, }\lambda = \int N_0 S_0(E) e^{-\int \mu(\xi, \eta, E) \mathrm{d}\eta} dE \tag{2.4}$$
</div>

To simplify it and connect it directly to the Radon transform, we will transform this equation using the Mean Value Theorem.

{% include start-box.html class="math-box"%}
$$\begin{split}
\lambda &= \int_{E_1}^{E_2} N_0 S_0(E) e^{-\int \mu(\xi, \eta, E) \mathrm{d}\eta} dE \\
&= e^{-\int \mu(\xi, \eta, \hat{E}_i) \mathrm{d}\eta} \int_{E_1}^{E_2} N_0 S_0(E) dE \\
&= e^{-\int \mu(\xi, \eta, \hat{E}_i) \mathrm{d}\eta} \lambda_0, 
\end{split} \tag{2.5}$$

where $\lambda_0 = N_0$ is the expected number of incident photons and $E_1 \le \hat{E}_i \le E_2$.
...
Here, $E_1$ and $E_2$ are the minimum and maximum photon energies, $\hat{E}_i$ is the effective energy for $i$-th line that lie between the $E_1$ and $E_2$ and $\lambda_0$ is equal to $N_0$, the expected number of incident X-ray photons.
By taking the natural logarithm and rearranging the equation, we can isolate the term we are truly interested in:

{% include start-box.html class="math-box"%}

$$-\ln \left( \frac{\lambda}{\lambda_0} \right) = \int \mu(\xi, \eta, \hat{E}_i) \mathrm{d}\eta \tag{2.6}$$

 {% include start-box.html class="math-box-inner" font_size="0.8em" %}
 This equation is the foundation of CT reconstruction:
...
In other words, the integral of the attenuation coefficient over a specific line ($\xi_i$ in $\xi-\eta$ coordinate) through the object, which is the Radon transform, is equal to the negative natural logarithm of the ratio of transmitted photons to incident photons.
...
<div class="math-box" style="padding-top: 0.2em;">
$$\text{Practical Radon transform}: -\ln \left( \frac{\lambda + \epsilon}{\lambda_0} \right) = \int \mu(\xi, \eta, \hat{E}_i) \mathrm{d}\eta \tag{2.7}$$
</div>

Up to this point, we have successfully linked the incident photons to the Radon transform, which represents attenuation. 
However, there is still one part that needs more explanation: the effective energy, $\hat{E}_i$, which is assumed to lie between the minimum and maximum photon energies. 
...
<div class="math-box" style="padding-top: 0.2em;">
$$p_i = -\ln \left( \frac{\lambda_i + \epsilon_i}{\lambda_0} \right) \approx \int \mu(\xi, \eta, \hat{E}_i) \mathrm{d}\eta \tag{2.8}$$
</div>
...
This inconsistency—where our model assumes a single $\hat{E}_i$ but reality provides a path-dependent one—leads to errors during image reconstruction. 
