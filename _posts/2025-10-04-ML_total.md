...
# W1-2 (Introduction to deep learning and modern AI)
...
# W2-1 (Deep Learning Theory Basics with Intuitive Insights)
{% include start-box.html class="math-box"%}
...
# W2-2 (Pytorch basics and Goal of Deep Learning)
{% include start-box.html class="math-box"%}
...
$$\mathcal{N}(\theta | \mu, \sigma^2\mathbf{I}) = \frac{1}{(2\pi\sigma^2)^{d/2}} \exp\left( -\frac{1}{2\sigma^2}\Vert\theta - \mu\Vert^2 \right)$$

Data term: the residual should follows zero-mean Gaussian.

$$p(\mathcal{D} | \theta) \propto \exp\left( -\frac{1}{2\sigma^2}\Vert\vec{y} - f_{\theta}(\vec{x})\Vert^2 \right)$$

and also assume zero-mean Gaussian prior

$$p(\vec{\theta}) = \mathcal{N}(\vec{\theta} | \mathbf{0}, \frac{1}{\lambda}\mathbf{I})$$
...
Because this small kernel (filter) is reused and **shared** across the entire image, it can learn to extract features effectively with far fewer parameters. The MNIST example ($1\times 28 \times 28$) from the lecture material confirms this, showing that the simpler CNN model uses fewer parameters than the more complex MLP model (CNN: 421,642 vs. MLP: 535,818).

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-10-04-ML_total/Fig_44.png" alt="" 
       style="width: 90%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   </figcaption>
</figure> 

- MLP ($784 \rightarrow 512 \rightarrow 256 \rightarrow 10$)
  - Linear(784,512): $784\times 512 = 501,408$ + 512(bias) = 401,920
  - Linear(512,256): $512 \times 256 = 131,072$ + 256(bias) = 131,328
  - Linear(256,10): $256 \times 10 = 2,560$ + 10(bias) = 2,570
  - total: 535,818
- SmallCNN
  - Conv2d($1 \rightarrow 32, 3\times 3$): $3 \times 3 \times 1 \times 32$ = 288 + 32(bias) = 320
  - Conv2d($32 \rightarrow 64, 3\times 3$): $3 \times 3 \times 32 \times 64$ = 18,432 + 64(bias) = 18,496
  - Flatten output: $67 \times 7 \times 7$ = 3136
  - Linear(3136,128): $3136 \times 128$ = 401,408 + 128(bias) = 401,536
  - Linear(128,10): $128 \times 10$ = 1,280 + 10(bias) = 1,290
  - total: 421,642 
...
**The Result: Translation Equivariance**<br>
...
**Inductive Bias = Powerful Regularization**<br>
...
**Step 1: Patch Partitioning (Batch,3,224,224) $\rightarrow$ (Batch,196,768)**
- **Process**: First, the input image is divided into a grid of non-overlapping small square pieces, called patches. And all patches are flattend into 1D vector.
...
**Step 2: Linear Projection (Patches $\rightarrow$ Tokens), (Batch,196,768) $\rightarrow$ (Batch,196,384)**
...
**Step 3: Add CLS Token and Positional Encoding, (Batch,196,384) $\rightarrow$ (Batch,197,384)**
...
**Step 4: Transformer Encoder, , (Batch,197,384) $\rightarrow$ (Batch,197,384)**
...
**Step 5: Classification Head, (Batch,197,384) $\rightarrow$ (Batch,384) $\rightarrow (Batch,200)$**
...
# W5-1 (Attention and Transformer)
{% include start-box.html class="math-box"%}
...
Here, weighting matrix $\Omega_q, \Omega_k, \Omega_v, (384,384) \text{matrix}$ are same for all 197 tokens.
{% include end-box.html %}
