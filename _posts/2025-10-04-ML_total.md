...
- **Process**: Each 2D image patch from Step 1 is flattened into a long 1D vector (e.g., a 16x16x3 patch becomes a 768-dimensional vector). This vector is then passed through a Linear **Projection** layer, which transforms it into a '**patch embedding**' vector of a consistent size.
...
- **Add [CLS] Token**: A special, learnable **[CLS] (Classification) Token** is added to the very beginning of the patch embedding sequence. This token is designed to aggregate information from the entire image. The sequence now consists of **197 tokens** in total.
...
- **Step 2**: The patches are converted into vector embeddings.
...
$$sa_n[\mathbf{x}_1, \dots, \mathbf{x}_N] = \sum_{m=1}^{N} a[\mathbf{x}_m, \mathbf{x}_n]v_m$$
