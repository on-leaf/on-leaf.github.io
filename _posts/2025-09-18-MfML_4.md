...
$$\langle \vec{x}, \vec{y} \rangle = \left<\sum_{i=1}^{n}\psi_i \vec{b}_i, \sum_{j=1}^{n}\lambda_j \vec{b}_j \right> = \sum_{i=1}^{n}\sum_{j=1}^{n}\psi_i <\vec{b}_i, \vec{b}_j>\lambda_j = \hat{\vec{x}}^{\top}A \hat{\vec{y}} \tag{2.4}$$
