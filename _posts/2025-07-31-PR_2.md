...
<div class="indented-paragraph" markdown="1">
Charge sharing makes the spatial resolution worse because spill-in charge sharing makes the effective pixel size bigger, especially at lower energies
</div>
...
# 3. Results
{% include start-box.html class="math-box"%}
## 3-1) Density images of two basis functions, single-pixel measurements
The results of the single-pixel acquisitions for density images of two basis functions are discussed in this section.

### 3-1-1) The number of counters($N_C$) and the number of coincidence counters ($N_{CC}$)
Figure 4 and Tables 1-2 show the normalized CRLB of water thicknesses with different $N_C$ and $N_{CC}$ with either eightor four neighbor pixels
<div class="indented-paragraph" markdown="1">
- For Fig.4, left, and Table 1
: CRLBs were normalized by the CRLB of the current dual-energy PCD ($N_C$=2 and $N_{CC}$=0, compare with standard techniques in usual hospital)
- For Fig.4, right, and Table 2
: CRLBs were normalized by the CRLB of the current PCD ($N_{CC}$=0, compare with MEICC-on and MEICC-off under same condition)

The CRLBs of bone thickness estimation were similar to those of water estimation, hence, not presented.
</div>


<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250810185010374.png" alt="Illustration of photoelectric interaction" 
       style="width: 60%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.4. The CRLB of water thickness with different numbers of energy windows $N_C$ with single-pixel measurements.
   </figcaption>
</figure> 

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250810185311653.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.1. The mean CRLB of water estimates normalized by that of the current dual-energy PCD ($N_C$=2 and $N_{CC}$=0).
   </figcaption>
</figure> 

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250810185330684.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.2. The mean CRLB of water estimates normalized by the corresponding CRLB of the current PCD ($N_{CC}$=0) with the same $N_C$.
   </figcaption>
</figure> 
...
[Fig.4. left]<br><br>
  {% include start-side-by-side.html
  image_src="../images/2025-07-31-PR_2/image-20250810193730483.png"
  image_alt=""
  image_width="40%"
  margin_top = "-4em"
  margin_bottom = "-4em"
  %}
  - The CRLB of the ideal PCD improved (i.e., decreased) from $N_C$=2 to $N_{C}$=4, then stayed relatively constant while increasing $N_C$.
  - With $N_{CC}$=0, the CRLBs improved as increasing $N_C$ in general, although changes became smaller with larger $N_C$ and the CRLB of $N_C$=4 was worse (larger) than that of $N_C$=3.
    <div class="indented-paragraph" markdown="1">
    The higher CRLB of $N_C$=4 than $N_C$=3 may be affected either threshold energies or charge sharing or both. (Exact reason is not clear)
    </div>
  - MEICC ($N_{CC}=N_{C}^{2}$) improved the CRLB
    {% include end-side-by-side.html %}
  {% include end-box.html %}
...
[Fig.4. right]<br><br>
  {% include start-side-by-side.html
  image_src="../images/2025-07-31-PR_2/image-20250810210837000.png"
  image_alt=""
  image_width="40%"
  margin_top = "-4em"
  margin_bottom = "-4em"
  %}
  - MEICC ($N_{CC}=N_{C}^{2}$) improved the CRLB for water thicknesses when $N_C$=4-12.
    <div class="indented-paragraph" markdown="1">

    - Only 5.7-16.4% worse than those of the ideal PCD
    - The CRLB of one coincidence counter ($N_{CC}$=1) is 37.3-45.6% worse than the ideal PCD
    </div>
    {% include end-side-by-side.html %}
  {% include end-box.html %}
...
{% include start-box.html class="math-box-inner" font_size="0.8em" %}
[The number of neighboring pixels (eight or four)]
- At $N_C \ge 4$, eight neighbors provieded ~10% better CRLB values for $N_{CC}=N_{C}^{2}$ and ~7% better for $N_{CC}=1$ than four.
- As seen in Fig.4, standard deviation of CRLB values were quite small.
  <div class="indented-paragraph" markdown="1">
  All of the 15,000 noise realizations were used to compute one CRLB value for each setting in the following
  </div>
{% include end-box.html %}

{% include start-box.html class="math-box-inner" font_size="0.8em" %}
[The threshold energies]<br>
The CRLBs with various threshold energies were studied with PCDs with $N_C$=4 and eight neighbors.
  <div class="indented-paragraph" markdown="1">
  - $E_1$ was fixed at 20keV
  - $E_2$ was set at either 30, 40, 50, 60 keV
  - $E_3, E_4$ were swept through  while satisfying a condition $E_2 \lt E_3 \lt E_4 \lt 140keV$

  Uniform window width is used and all of the CRLBs were normalized by the CRLB of the current dual-energy PCD.
  </div>

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250810215002570.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.5. The normalized CRLBs of water thickness with $N_C$=4 and different threshold energies with single-pixel measurements
   </figcaption>
</figure> 

Fig.5 presents the CRLB of the ideal PCD, $N_{CC}$=0,1 and $N_{C}^{2}$.
They are looks similar, however absolute values were different and MEICC ($N_{CC}=N_{C}^{2}$) was the least sensitive to threshold energies (Figs. 5J-5L).
<div class="indented-paragraph" markdown="1">
MEICC has wide line width which means that it keeps relatively constant CRLB about variation of energies. (Stable for energy)
</div>
The best CRLBs (cross marker) were as follows:
<div class="indented-paragraph" markdown="1">
- The ideal PCD: 0.224
- The current dual-energy PCD: 0.650
- One coincidence counter ($N_{CC}$=1): 0.328
- MEICC: 0.255

These values are better than the corresponding CRLBs with $N_C$=6 with a uniform energy window width (see Table.1).
</div>

The minimum CRLB for MEICC was found when three of the four treshold energies were at or below 45keV (Fig.5J), while the other PCDs was obtained when three out of four threshold energies were set at or above 50keV.
<div class="indented-paragraph" markdown="1">
It appeared that MEICC used the information obtained by lower threshold energies differently from the othre PCDs. The reason is that charge sharing populates the low-energy bins with numerous spurious events. While other techniques cannot distinguish these false events from genuine ones, MEICC possesses a unique advantage: its spectral coincidence information. By recording which specific energy windows fired simultaneously, MEICC can accurately identify these spurious low-energy events and trace them back to their high-energy origins, effectively using this 'corrupted' data for correction.
</div>

The number of coincidence counters with $\gt$30 counts (a reasonable statistical power) was 15 out of 16 with optimal energy thresholds, while it was only 10 out of 16 with the uniform energy window width (see Table.3).
{% include end-box.html %}

### 3-1-2) The number of coincidence counters ($N_{CC}$)
With $N_C$=4, eight neighbors, and the optimal threshold energies studied above, the number of coincidence counters $N_{CC}$ for MEICC was reduced from 16 to 9 by combining energy windows of neighboring pixels for coincidence counteres as shown in Fig.6. 

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811125224301.png" alt="Illustration of photoelectric interaction" 
       style="width: 60%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.6. Coincidence counter indices with $N_C$=4
   </figcaption>
</figure> 

Multiple energy windows of neighboring pixels were bundled and treated as one wider energy window (see Fig.6B). 
<div class="indented-paragraph" markdown="1">
When a count is registered at energy window 1 of the POI via spill-in charge sharing it does not critical to know exactly which energy window of the neighboring pixels a count is also registered. 
In this reason, authors integrated some pixels.
This design is choosed because some energy windows either had very small counts or seemed to provide minimal effects on the material decomposition for the POI.
</div>
In fact, the CRLB increased slightly from 0.255 for $N_{CC}$=16 to 0.269 for $N_{CC}$=9 (5.5% change).
In comparison, the CRLB with four neighbors was 0.284 for $N_{CC}$=16 and 0.297 for $N_{CC}$=9 (11.4% and 10.5% change).

The number of neighboring pixels had a larger impact on CRLBs than the number of coincidence counters in this case.

## 3-2) Mono-energetic CT imaging, single-pixel measurements
Figure 7A and Table 4 show the CRLB of mono-energetic CT images at 60keV with the single-pixel measurements normalized by the CRLB of the current dual-energy PCD ($N_C$=2 and $N_{CC}$=0)

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811135325884.png" alt="Illustration of photoelectric interaction" 
       style="width: 60%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.7. The CRLB of line integrals in mono-energetic CT imaging (A) and gold thicknesses in K-edge imaging (B) with different numbers of energy windows $N_C$
   </figcaption>
</figure> 

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811135507556.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.4. The mean CRLB of mono-energetic CT images normalized by that of the current dual-energy PCD ($N_C$=2 and $N_{CC}$=0)
   </figcaption>
</figure> 

Table 5 shows the CRLB normalized by the corresponding CRLB of the current PCD with the same $N_C$.

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811135351634.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.5. The mean CRLB of mono-energetic CT images normalized by the corresponding CRLB of the current PCD ($N_{CC}$=0) with the same $N_C$
   </figcaption>
</figure> 

The differences in CRLBs for mono-energetic CT imaging among different PCD types were smaller than the differences for density images of the two basis functions.
<div class="indented-paragraph" markdown="1">
It is because mono-energetic CT imaging is a less demanding spectral task compared to basis material density imaging. Negatively correlated noise in water and bone densities tended to suppress noise when mono-energetic CT images were synthesized. This is not meaning that MEICC getting worse.
</div>

## 3-3) K-edge imaging, single-pixel measurements
Figure 7B and Table 6 present the CRLB of gold thickness with the single-pixel measurements normalized by the CRLB of the current PCD with four energy windows ($N_C$=4 and $N_{CC}$=0).

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811143538923.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.6. The mean CRLB of K-edge images normalized by that of the current four-energy PCD ($N_C$=4 and $N_{CC}$=0)
   </figcaption>
</figure> 

Table 7 shows the CRLB normalized by the corresponding CRLB of the current PCD with the same $N_C$.
<div class="indented-paragraph" markdown="1">
The threshold energies include one at teh K-edge of gold(80keV) when the number of energy windows ($N_C$) was 4,6,8, or 12.
</div>
<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-31-PR_2/image-20250811143551875.png" alt="Illustration of photoelectric interaction" 
       style="width: 80%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Table.7. The mean CRLB of K-edge images normalized by the corresponding CRLB of the current PCD ($N_{CC}$=0) with the same $N_C$
   </figcaption>
</figure> 

Among the three imaging tasks, K-edge imaging had the largest difference between the current PCD and the ideal PCD.
<div class="indented-paragraph" markdown="1">
K-edge imaging is the most demanding spectral task among the three because jump signal of K-edge is vurnurable to noise
</div>
MEICC had the CRLB much closer to the ideal PCD than others.
It demonstrates that the spectral information on the charge sharing recorede by MEICC was critical for spectral tasks.

## 3-4) Super-pixel measurements
Table 1-2 present the CRLB of water thickness estimation with super-pixel  measurements.
Table 4-5 present the CRLB of mono-energetic CT images with super-pixel measurements.
Tables 6-7 present the CRLB of K-edge imaging with super-pixel measurements.

<div class="indented-paragraph" markdown="1">
Compared with single-pixel measurements in Table 1,
- The CRLB values of both MEICC ($N_{CC} = N_{C}^{2}$) and one coincidence counter ($N_{CC}$=1) become worse
- The difference between 8 neighbors and 4 neighbors becomes large
- The difference between MEICC and one coincidence counter becomes small
- The CRLB of the ideal PCD and the current dual-energy PCD ($N_C$=2 and $N_{CC}$=0) with super-pixel measurements were comparable to the corresponding CRLB with single-pixel measurements
</div>

<div class="indented-paragraph" markdown="1">
Compared with single-pixel measurements in Table 3,
- The CRLB values of the ideal PCD and 8 neighbors with both MEICC and one coincidence counter become better
- The CRLB of the current dual-energy CT and 4 neighbors becomes worse
- The difference between MEICC and one coincidence counter become small
</div>

<div class="indented-paragraph" markdown="1">
Compared with single-pixel measurements in Table 6,
- The CRLB values of the ideal PCD and the current dual-energy CT remained comparable to single-pixel measurements
- The CRLB values of the MEICC become worse
- The CRLB values of one coincidence counter become better
</div>

Authors said that the reasons for the above discussed differences from single-pixel measurements and the different degrees of changes among PCDs are not clear.
They speculates that the reason of these differences come from difference in optimal threshold energies between two test. 
{% include end-box.html %}

# 4. Discussion and Conclusions
{% include start-box.html class="math-box"%}
The MC study showed that the performance of MEICC is promissing. 
For two-basis material decomposition with four or more energy window, the CRLB of MEICC were 36-36% of the current PCD with the same number of energy windows and were very close to those of the ideal PCD (at 33-36% levels).

The study also showed the follwing results:
<div class="indented-paragraph" markdown="1">

- The threshold energies significantly affected the CRLB
- The best CRLB for MEICC was achieved when most of the thresholds were set at lower energies (Opposite to the other PCDs)
- Presence of eight neighbors was ~10% better than 4 neighbors
  <div class="indented-paragraph" markdown="1">
  The number of coincidence counters might be reduced to about a half of $N_{C}^{2}$ with a limited impact on the CRLB
  </div>
</div>

MEICC performed well for K-edge imaging with CRLBs at 30-43% of the current PCD with the same number of energy windows.
<div class="indented-paragraph" markdown="1">
K-edge imaging needs more accurate sepctral information than two-basis material decomposition and MEICC was effective
</div>

The difference between one coincidence counter and MEICC shows the difference between the intensity information of charge sharing one coincidence counter records and the spectral information MEICC captures.
Authors says that all components in the MEICC design are currently available. 
However one may be concerned with heat, power, consumption, and space needed for MEICC.

Also, authors says that the study has limitations. 
Threshold energies were not optimized, and they have to be done for each clinical task.
The number of coincidence counters ($N_{CC}$) may be decreased and the way to bundle and connect energy windows was not studied systematically.
Other conditions - x-ray spectra, pixel sizes, electronic noise level - would affect the CRLB and the effectiveness of MEICC. 
And since this study focused on charge sharing, pulse pileup must be included when different pixel sizes are stuided.
Authors speculate that PCDs with MEICC may perform best with small size pixel when pulse pileup is negligible.
Also, they said that algorithms for MEICC shuld be deveoped more.  
{% include end-box.html %}