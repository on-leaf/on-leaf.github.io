...
Unfortunately, there is a practical difficulty one is faced with the simple direct inverse Radon transform.

<div class="indented-paragraph" markdown="1">
- Due to the limited number of projections, the values of the Fourier transform $F(u,v)$ of the attenuation coefficients $f(x,y)$ are only known for a limited number of measured radial lines in the $(u,v)$ space
- The distribution of the polar spectral data in the Cartesian coordinate system
</div>

The main problem is the distribution. 
Below figure shows the problem.

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-08-CT_6/image-20250729175321541.png" alt="Illustration of photoelectric interaction" 
       style="width: 90%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.2.3 Radially arranged values $P_{\gamma}(q)$ and Regridding of the spectral projection data.
   </figcaption>
</figure> 

Since the fast Fourier transform (FFT) needs data lying on a regular rectangular grid, the data $P_{\gamma}(q)$, which are arranged in circles, must be defined on a Cartesian grid before inverse transformation. 
This process is called regridding.

In order to perform the direct inverse Radon transformation using an FFT algorithm, the Cartesian grid has to be created from polar configuration of spectral data via an appropriate interpolation (e.g, nearest-neighbor, bilinear, etc.).
As a direct consequence of the radial data arrangement, the density of measured spectral data in the $(u,v)$ space decreases for higher freqeuncies. 

<div class="indented-paragraph" markdown="1">
The interpolation error increases at high spatial frequencies.
This effect leads to a degradation of image.
</div>

In this reason, another reconstruction method called "filtered backprojection" is used in practice.

# 3. Simple Backprojection & Filtered Backprojection
As seen before, the Fourier slice theorem induces a general direct reconstruction procedure.
However, due to the problems of Cartesian regridding, a different reconstruction method is used in practice.
This method is called "filtered backprojection (FBP)".

Before see what is the FBP, we need to think about "simple" backprojection not the "filtred" backprojection.

## 3-1) Simple Backprojection
One might think that the reconstruction image could be obtatined by a simple backprojection of the measured projection profiles $P_{\gamma}(\xi)$.
<div class="indented-paragraph" markdown="1">
Smear back the profile values in the direction from which the radiation came.
</div>

This process can be modeled by the following equation:

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \int_{0}^{\pi} p_{\gamma}(\xi) \mathrm{d} \gamma \\ 
                               &= \int_{0}^{\pi} p_{\gamma}\left( x \cos (\gamma) + y \sin (\gamma) \right) \mathrm{d} \gamma  \end{split} \tag{3.1}$$</center>
{% include end-box.html %}

In fact, this backprojection is not an adequate reconstruction for the original image $f(x,y)$.
Because each point in the image grid receives non-negative contributions from all other points of the original image.

<div class="indented-paragraph" markdown="1">
Since the projection profile $p_{\gamma}(\xi)$ is a non-negative function, the simple backprojection smears back non-negative values over the entire image, even on the points where $f(x,y)=0$.
</div>

Let's look at the backprojection process in detail as follows

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \int_{0}^{\pi} p_{\gamma}\left( x \cos (\gamma) + y \sin (\gamma) \right) \mathrm{d} \gamma \\ 
                               &= \int_{0}^{\pi} \mathop{\int\int}\limits_{\mathbf{r} \in \mathbf{L}} f(\mathbf{r}) \mathrm{d}\mathbf{r} \mathrm{d} \gamma \\
                               &= \int_{0}^{\pi} \mathop{\int\int}\limits_{\mathbf{r} \in \mathbb{R}^2} f(\mathbf{r}) \delta(\mathbf{r} - \mathbf{L}) \mathrm{d}\mathbf{r} \mathrm{d} \gamma
                               \end{split} \tag{3.2}$$</center>
{% include end-box.html %}

The line $\mathbf{L}$, on which the backprojection takes place, can be substituted by the $(\mathbf{r}' ^{T} \cdot \mathbf{n}_{\xi}) = \xi$ 

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \int_{0}^{\pi} \mathop{\int\int}\limits_{\mathbf{r}' \in \mathbb{R}^2} f(\mathbf{r}') \delta\left( (\mathbf{r}^T \cdot \mathbf{n}_{\xi}) - (\mathbf{r}'^T \cdot \mathbf{n}_{\xi}) \right) \mathrm{d}\mathbf{r}' \mathrm{d}\gamma \\ 
                               &= \mathop{\int\int}\limits_{\mathbf{r}' \in \mathbb{R}^2} f(\mathbf{r}') \left( \int_{0}^{\pi} \delta\left( (\mathbf{r}^T \cdot \mathbf{n}_{\xi}) \mathrm{d}\gamma \right) \right) \mathrm{d}\mathbf{r}' \end{split} \tag{3.3}$$</center>
{% include end-box.html %}

As indicated in Fig3.1, let $\varphi$ be the angle between the difference vector $(\mathbf{r} - \mathbf{r}')$ and the x-axis.

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-08-CT_6/image-20250804134952525.png" alt="Illustration of photoelectric interaction" 
       style="width: 60%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.3.1 Reconstruction geometry for simple backprojection.
   </figcaption>
</figure> 

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \mathop{\int\int}\limits_{\mathbf{r}' \in \mathbb{R}^2} f(\mathbf{r}') \left( \int_{0}^{\pi} \delta\left( \vert \mathbf{r}-\mathbf{r}' \vert \cos (\varphi - \gamma) \right) \right) \mathrm{d}\mathbf{r}' \\
                               &= \mathop{\int\int}\limits_{\mathbf{r}' \in \mathbb{R}^2} f(\mathbf{r}') \left( \int_{0}^{\pi} \frac{\delta(\gamma - \gamma_0)}{\vert \mathbf{r} - \mathbf{r}' \vert \vert \sin (\pm \frac{\pi}{2}) \vert} \mathrm{d}\mathbf{r}' \right) \end{split} \tag{3.4}$$</center>
{% include end-box.html %}

The denominator of the inner integral in (3.4) is constant in terms of integration along the projection angle, $\gamma$. 
The integration of the $\delta$-distribution in the numerator results in the value 1, so that

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \mathop{\int\int}\limits_{\mathbf{r}' \in \mathbb{R}^2} f(\mathbf{r}') \frac{1}{\vert \mathbf{r} - \mathbf{r}' \vert} \mathrm{d}\mathbf{r}' \\
                               &= \int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty} f(x', y') \frac{1}{\vert (x-x', y-y') \vert} \mathrm{d}x' \mathrm{d}y' \end{split} \tag{3.5} $$</center>
{% include end-box.html %}

Equation (3.5) represents a convolution of the original image $f(x,y)$ with the function 

{% include start-box.html class="math-box"%}
<center>$$h(x,y) = \frac{1}{\vert (x,y) \vert} \tag{3.6}$$</center>
{% include end-box.html %}

Thus,

{% include start-box.html class="math-box"%}
<center>$$g(x,y) = f(x,y) \ast h(x,y) \tag{3.7}$$</center>
{% include end-box.html %}

In fact, $h(x,y)$ is the point-spread function (PSF) of the imaging system.

<figure style="display: flex; flex-direction: column; align-items: center; margin-top: 0.5em; margin-bottom: 0.5em;">
  <img src="../images/2025-07-08-CT_6/image-20250805150136185.png" alt="Illustration of photoelectric interaction" 
       style="width: 60%; height: auto;">
   <figcaption style="font-size: 20px; margin-top: -0.5em;">
   Fig.3.2 (a) The line density around the object that is to be reconstructed decreases with the distance from the point. (b) The point-spread function of a single point.
   </figcaption>
</figure> 

The PSF answers the question of how the image of an ideal object point is blurred or spread by the imaging system due to the backsmearing process.\
The density of the lines in $g(x,y)$ around the point in $f(x,y)$ geometrically decreases with $\vert \mathbf{r}^{-1} \vert$.

This result is explicitly obtained by substituting the function $f(x,y)$ with the $\delta$-distribution in (3.5)

{% include start-box.html class="math-box"%}
<center>$$\begin{split} g(x,y) &= \int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty} f(x',y') \frac{1}{\vert (x-x',y-y') \vert} \mathrm{d}x' \mathrm{d}y' \\
                               &= \int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty} \delta(x',y') \frac{1}{\vert (x-x',y-y') \vert} \mathrm{d}x' \mathrm{d}y' \end{split} \tag{3.8}$$</center>
{% include end-box.html %}

With the sifting property of the $\delta$-distribution, 

{% include start-box.html class="math-box"%}
<center>$$ g(x,y) = \frac{1}{\vert (x,y) \vert} \tag{3.9}$$</center>
{% include end-box.html %}

In this case the reconstructed image $g(x,y)$ itself is the representation of the PSF.
Any point of the object to be reconstructed is blurred by $h(x,y)$. 
In fact, adqeuate reconstruction strategies can be found that include a suitable deconvolution of the image, thus compensating for the blurring effects seen in naive backprojection.