...
* $(\mathbb{Z}, +)$ is an Abelian group ($\mathbb{Z}$ is integer set). 
* $(\mathbb{N} \cup {0}, +)$ is not a group $\mathbb{N} \cup {0}$ is positive integer set. 
    <div class="indented-paragraph" markdown="1">
...
* $(\mathbb{R}, \cdot)$ is not a group since 0 does not possess an inverse element ($\mathbb{R}$ is real number set).
* $(\mathbb{R} \setminus \{0\}, \cdot)$ is Abelian.
* $(\mathbb{R}^n, +), (\mathbb{Z}^n, +), n \in \mathbb{N}$ are Abelian if $+$ is defined componentwise, i.e.,<br> <center><br>$(x_1, \dots, x_n) + (y_1, \dots, y_n) = (x_1 + y_1, \dots, x_n + y_n). \quad (4.1)$</center> <br>Then, $(x_1, \dots, x_n)^{-1} := (-x_1, \dots, -x_n)$ is the inverse element and $e = (0, \dots, 0)$ is the neutral element.
...
<div class="indented-paragraph" markdown="1">
We can think of the inner operation as a form of addition, and the outer operation as a form of scailing, but the inner/outer operations have nothing to do with inner/outer products.
</div>
...
Consider a vector space _V_ and a finite number of vectors $\vec{x}_1, \dots, \vec{x}_k \in V$.
Then every $\vec{v} \in V$ of the form 
...
## 6-2) Rank
The number of linearly independent columns of a matrix $A \in \mathbb{R}^{m \times n}$ equals the number of linearly independent rows and is called the **_rank_** of $A$ and is denoted by rk($A$).

{% include start-box.html class="math-box-inner" font_size="0.8em"%}
The rank of a matrix has some important properties:
- $\text{rk}(A) = \text{rk}(A^{\top})$
- The columns of $A \in \mathbb{R}^{m \times n}$ span a subspace $U \subseteq \mathbb{R}^m$ with $\text{dim}(U) = \text{rk}(A)$.<br> This subspace $U$ is called **_image_** or **_range_**
- The rows of $A \in \mathbb{R}^{m \times n}$ span a subspace $W \subseteq \mathbb{R}^n$ with $\text{dim}(W) = \text{rk}(A^{\top})$
- For all $A \in \mathbb{R}^{n \times n}$ it holds that $A$ is regular (invertible) if and only if $\text{rk}(A) = n$
- For all $A \in \mathbb{R}^{m \times n}$ and all $b \in \mathbb{R}^m$ it holds that the linear equation system $Ax = b$ can be solved if and only if $\text{rk}(A) = \text{rk}(A\vert b)$, where $A\vert b$ denotes the augmented system
- For $A \in \mathbb{R}^{m \times n}$ the subspace of solutions for $Ax = 0$ possesses dimension $n - \text{rk}(A)$<br> This subspace is called **_kernel_** or the **_null space_**
- A matrix $A \in \mathbb{R}^{m \times n}$ has **_full rank_** if its rank equals the largest possible rank for a matrix of the same dimensions<br>This means that the rank of a full-rank matrix is the lesser of the number of rows and columns, i.e., $\text{rk}(A) = \min(m, n)$<br> A matrix is said to be **_rank deficient_** if it does not have full rank
{% include end-box.html %}
{% include end-box.html %}
